...
public class LicenseManager {
    ...
    public boolean validate() {
        ...
        String serverUrl = plugin.getConfig().getString("license.server-url", "");
        ...
        // Check revocation list if server URL is set
        if (!serverUrl.isEmpty()) {
            try {
                String checkUrl = serverUrl.replaceAll("/$", "") + "/check/" + key;
                ...
                if (conn.getResponseCode() == 200) {
                    ...
                    if (response != null && response.contains("\"valid\":false")) {
                        printBanner(key, "Revoked");
                        return false;
                    }
                }
            } catch (Exception e) {
                // If server unreachable, fall through to offline validation
                plugin.getLogger().warning("[EclipseAC] Could not reach license server, using offline validation.");
            }
        }
    ...
    private void saveLockedIp(String key, String ip) {
        try {
            ...
            plugin.getLogger().warning("[EclipseAC] Could not save license lock.");
        }
    }
    ...
    private static String hmac(String data, String secret) {
        try {
            ...
            for (int i = 0; i < 8; i++) sb.append(String.format("%02X", raw[i]));
            return sb.toString();
